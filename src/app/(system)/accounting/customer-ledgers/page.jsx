...
import { Loader2, ArrowUpRight, ArrowDownLeft, User, RotateCcw, FileText, CalendarIcon, Plus, Search } from "lucide-react";
            ...
            <Card className="border-slate-200 shadow-sm overflow-hidden rounded-xl bg-slate-50/30">
                <CardContent className="p-4 flex flex-col md:flex-row gap-4 items-end">
                    <div className="w-full md:w-64 space-y-1.5">
                        <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Search Customer</label>
                        ...
                        >
                            ...
                            <SelectContent className="rounded-xl">
                                {customers
                                    ...
                                    .map(c => (
                                        <SelectItem key={c.id} value={String(c.id)} className="text-xs font-bold uppercase tracking-wider">{c.name}</SelectItem>
                                    ))
                                }
                            </SelectContent>
                        </Select>
                        ...
                        <CardHeader className="bg-gradient-to-br from-blue-700 to-blue-900 text-white border-b-0 pb-12 relative overflow-hidden">
                            ...
                            <div className="flex justify-between items-start relative z-10">
                                <div className="space-y-1">
                                    ...
                                    <CardTitle className="text-2xl font-black uppercase tracking-tight leading-none">Statement of Account</CardTitle>
                                    <CardDescription className="text-blue-100/70 font-medium">
                                        Analysis for <span className="text-white font-black">{customers.find(c => String(c.id) === selectedCustomer)?.name}</span>
                                    </CardDescription>
                                </div>
                            ...
                            <Table>
                                <TableHeader className="bg-slate-50/50">
                                    <TableRow className="hover:bg-transparent border-slate-100 italic-none">
                                        ...
                                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-5 px-6 text-slate-400">Transaction Reference</TableHead>
                                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-5 px-6 text-right text-slate-400">Debit (Owed)</TableHead>
                                        <TableHead className="text-[10px] font-black uppercase tracking-widest py-5 px-6 text-right text-slate-400">Credit (Paid)</TableHead>
                    ...
                    <div className="space-y-6">
                        <Card className="border-none shadow-2xl bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white rounded-3xl overflow-hidden relative">
                            ...
                            <CardContent className="p-8 space-y-8 relative z-10">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Current Outstanding</p>
                                ...
                                <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/10">
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-lg font-black tracking-tight">LKR {parseFloat(totalPaid).toFixed(2)}</p>
                                    </div>
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-lg font-black tracking-tight">LKR {parseFloat(totalOwed).toFixed(2)}</p>
                                    </div>
                                </div>
                        ...
                        <Dialog open={paymentOpen} onOpenChange={setPaymentOpen}>
                            <DialogContent className="sm:max-w-md rounded-2xl">
                                <DialogHeader>
                                    <DialogTitle className="text-xl font-black uppercase tracking-tight">Record Customer Payment</DialogTitle>
                                    <DialogDescription className="font-medium text-slate-500">
                                        Record a payment received from {customers.find(c => String(c.id) === selectedCustomer)?.name}.
                                    </DialogDescription>
                                </DialogHeader>
                                <form onSubmit={handlePaymentSubmit} className="space-y-4 py-4">
                                    ...
                                    {paymentMethod === "cheque" && (
                                        <div className="grid gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 mt-1">
                                            <div className="grid grid-cols-4 items-center gap-4">
                                                <Label className="text-right text-[10px] uppercase font-black text-slate-400 tracking-widest">Bank</Label>
                                                <Input 
                                                    value={chequeDetails.bank_name} 
                                                    onChange={(e) => setChequeDetails({...chequeDetails, bank_name: e.target.value})}
                                                    placeholder="Bank Name" 
                                                    className="col-span-3 h-10 text-xs bg-white rounded-xl font-bold" 
                                                />
                                            </div>
                                            <div className="grid grid-cols-4 items-center gap-4">
                                                <Label className="text-right text-[10px] uppercase font-black text-slate-400 tracking-widest">Cheque #</Label>
                                                <Input 
                                            ...
                                            <div className="grid grid-cols-4 items-center gap-4">
                                                <Label className="text-right text-[10px] uppercase font-black text-slate-400 tracking-widest">Date</Label>
                                                <Input 
                                                    type="date"
                                    ...
                                    <DialogFooter className="mt-6">
                                        ...
                                        <Button type="submit" disabled={paymentLoading} className="bg-slate-900 text-white hover:bg-slate-800 rounded-xl px-8 h-11 font-black text-[10px] uppercase tracking-widest">
