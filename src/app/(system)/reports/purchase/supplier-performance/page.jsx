  ...
  return (
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Supplier Performance</h1>
          <p className="text-slate-500">Analytics on procurement volume and reliability by supplier.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-1 border-none shadow-sm bg-gradient-to-br from-blue-600 to-blue-700 text-white">
          ...
          <CardContent>
            {isLoading ? <Skeleton className="h-10 w-40 bg-blue-500" /> : (
                <>
                    ...
                    <div className="mt-6 pt-4 border-t border-blue-500/50 flex justify-between">
                        <div>
            ...
            <CardContent>
                <div className="flex items-center gap-8 h-32">
                    <div className="flex flex-col gap-2">
                        <span className="text-sm font-medium text-slate-500">Active Suppliers</span>
                        <span className="text-3xl font-black text-slate-900">{data.length}</span>
                    </div>
                    <div className="w-px h-16 bg-slate-100" />
                    <div className="flex flex-col gap-2">
                        <span className="text-sm font-medium text-slate-500">Total Purchase Volume</span>
                        <span className="text-3xl font-black text-slate-900">LKR {data.reduce((s,i) => s + i.totalPurchase, 0).toLocaleString()}</span>
                    </div>
                </div>
            </CardContent>
        </Card>
              ...
              {isLoading ? (
                ...
                filteredData.map((item) => (
                  <TableRow key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <TableCell className="text-slate-500">{item.company || '-'}</TableCell>
                    <TableCell className="text-center font-medium">
                        <Badge variant="secondary" className="bg-slate-100">{item.productCount}</Badge>
                    </TableCell>
