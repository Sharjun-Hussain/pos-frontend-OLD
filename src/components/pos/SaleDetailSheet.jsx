  ...
  return (
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[600px] flex flex-col h-full p-0 overflow-hidden border-l border-border/50">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div className="space-y-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] font-black tracking-[0.2em] text-emerald-100 uppercase">Transaction ID</span>
              </div>
            ...
            <div className="flex flex-col items-end gap-3">
              ...
              <div className="text-right">
                <p className="text-[10px] font-bold text-emerald-200 uppercase tracking-widest mb-1">Total Amount</p>
          ...
          <div className="p-8 space-y-10">
            ...
            <div className="grid grid-cols-2 gap-10">
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/30">
                    ...
                    <h4 className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Customer Profile</h4>
                </div>
                <div className="space-y-1">
                  <p className="text-base font-bold text-foreground">{sale.customer?.name || "Walk-in Customer"}</p>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 pb-2 border-b border-border/30">
                    ...
                    <h4 className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Payment Method</h4>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground/60 font-medium">via {sale.cashier?.name || 'Main POS'}</span>
                  </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-border/30">
                  <h4 className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-widest">Transaction Items</h4>
                    ...
                    {sale.items?.map((item, idx) => (
                      <TableRow key={idx} className="border-border/30 group hover:bg-muted/20 transition-colors">
                        ...
                        <TableCell className="py-4 text-center">
                          <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-full bg-background text-foreground text-[11px] font-bold">
            ...
            <div className="bg-muted/30/80 rounded-2xl p-6 space-y-4 border border-border/30">
              <div className="space-y-3 pb-4 border-b border-border/50">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-bold text-foreground">LKR {subtotal.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground font-medium">Instant Discount</span>
                  <span className="font-bold text-red-500">- LKR {discount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground font-medium">Applied Tax</span>
                  <span className="font-bold text-foreground">LKR {tax.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
              </div>

              <div className="flex justify-between items-center py-2">
                <div>
                  <h3 className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em] mb-1">Final Payable</h3>
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-black text-foreground tracking-tighter">LKR {payable.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
                <div className="text-right">
                  <h3 className="text-[10px] font-black text-muted-foreground/60 uppercase tracking-[0.2em] mb-1">Amount Paid</h3>
                  <Badge className="bg-emerald-500/10 text-emerald-500 border-emerald-500/20 text-sm font-black px-4 py-1">
              ...
              {balance > 0 && (
                <div className="pt-3 border-t border-border/50 flex justify-between items-center">
                  <span className="text-xs font-bold text-muted-foreground/60 uppercase tracking-widest">Change Return</span>
                  <span className="text-sm font-black text-emerald-500">LKR {balance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* --- Sticky Footer --- */}
        <div className="p-6 bg-card border-t border-border/30 flex gap-4 shrink-0 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
          <Button
            className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white h-12 rounded-xl font-bold gap-2 shadow-lg transition-all active:scale-[0.98]"
          ...
          <Button
            variant="outline"
            className="h-12 w-12 rounded-xl border-border/50 hover:bg-muted/30 hover:text-red-500 transition-all active:scale-[0.98]"
